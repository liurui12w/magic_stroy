ActiveAdmin.register Schedule do
  ...
  form do |f|
    f.inputs "日程/清单" do
      ...
      f.input :owntype, :label=> "类型", :as => :select, :collection => ["Schedule","DetailedList"], refreshes: :schedule_date, input_html: { data: { eq: 'DetailedList', action: 'hide', target: '.grp1' } }
      f.input :schedule_date, :label=> "日程Date", as: :date_time_picker,
                         picker_options: {
                           min_date: Date.current - 2.days,
                           timepicker: false
                         }
      f.input :title, :label=> "标题", :input_html => {:value => "这是标题"}
      f.input :remind_date, :label=> "提醒时间", as: :date_time_picker, wrapper_html: { class: 'grp1' }
      f.input :start_date, :label=> "开始时间", as: :date_time_picker, wrapper_html: { class: 'grp1' }
      f.input :end_date, :label=> "结束时间", as: :date_time_picker, wrapper_html: { class: 'grp1' }
